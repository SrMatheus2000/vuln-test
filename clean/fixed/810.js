function unique_name_442(options) {
    ...
    _parsePattern = function(pat) {
      ...
      if (m) {
        pars = (function() {
          var i, len1, results;
          results = [];
          for (i = 0, len1 = m.length; i < len1; i++) {
            x = m[i];
            results.push(x.slice(2));
          }
          return results;
        })();
    ...
    dispatch = function(req, res) {
      ...
      final_dispatch = function(req, res) {
        var args, home_page, homes, i, index, j, len1, len2, m, method, param, parsed, pathname, ref, route, selected_method, send_static_home;
        ...
        for (i = 0, len1 = selected_method.length; i < len1; i++) {
          ...
          if (m !== null) {
            if (route.params) {
              ...
              args = m.slice(1);
              ref = route.params;
              for (index = j = 0, len2 = ref.length; j < len2; index = ++j) {
                param = ref[index];
        ...
        if (pathname === "/") {
          homes = (function() {
            ...
            for (l = 0, len3 = ref1.length; l < len3; l++) {
              home_page = ref1[l];
              results.push("" + dispatch.static_route + path_tools.sep + home_page);
            }
            return results;
          })();
    ...
    dispatch.get_route_handler = function(path, method) {
      var chosen, i, j, key, len1, len2, object, objects, ref;
      if (!method) {
        ref = dispatch.routes;
        for (key in ref) {
          objects = ref[key];
        ...
        for (j = 0, len2 = chosen.length; j < len2; j++) {
          object = chosen[j];
          if (path.match(object.pattern)) {
            return {
              method: method,
    ...
    dispatch.cookie2obj = function(cookie_str) {
      var cookiePairs, eqIndex, i, key, len1, pair, ret, value;
      ret = {};
      cookiePairs = (function() {
        ...
        for (i = 0, len1 = ref.length; i < len1; i++) {
          pair = ref[i];
          results.push(pair.trim());
        }
        return results;
      })();
      for (i = 0, len1 = cookiePairs.length; i < len1; i++) {
        pair = cookiePairs[i];
    ...
    dispatch.memory_store = function(request, opcode, sessobj, cb) {
      ...
      switch (opcode) {
        case 'set':
          ...
          break;
        case 'get':
          if ((ref = !sid, indexOf.call(nsr_sessions, ref) >= 0) || (!nsr_sessions[sid])) {
            nsr_sessions[sid] = sessobj;
          }
          break;
        default:
    ...
    dispatch.getEnv = function(pathname, req, res) {
      var env, key, query_pairs, ref, val;
      env = {};
      ref = req.headers;
      for (key in ref) {
        val = ref[key];
    ...
    dispatch.scgi_pass = function(conn, request, response) {
      ...
      getData = function() {
        ...
        return client.on('end', function(data) {
          var contentType, contentTypeDone, headerSet, i, index, len1, line, lines, m, status, statusDone, writeThis;
          if (data) {
            retval += data;
          }
      ...
      return list_func(fpath, path).then(function(arrs) {
        ...
        context.files = (function() {
          ...
          for (i = 0, len1 = files.length; i < len1; i++) {
            file = files[i];
            if ((ref = mime_types[path_tools.extname(file.toLowerCase())]) != null ? ref.match(/image/) : void 0) {
              results.push({
    ...
    dispatch.static_directory = function(fpath, path, res, list_func) {
      ...
      return list_func(fpath, path).then(function(arrs) {
        var file, files, i, index, len1, obj, resp, stats;
    ...
    dispatch.render_template = function(template_string, context, keep_tokens) {
      ...
      if (sections) {
        for (i = 0, len1 = sections.length; i < len1; i++) {
          section = sections[i];
      ...
      for (index = j = 0, len2 = text_tokens.length; j < len2; index = ++j) {
        text_token = text_tokens[index];
        variable_tokens = text_token.match(variable_pattern_global);
        token_obj = {};
        if (variable_tokens) {
          for (l = 0, len3 = variable_tokens.length; l < len3; l++) {
      ...
      ret_arr = [];
      for (index = o = 0, len4 = text_tokens.length; o < len4; index = ++o) {
        ...
        if (section_tokens[index]) {
          ...
          if (section_type === false) {
            ...
            if (((property != null ? property.length : void 0) != null) && ((property != null ? property.constructor.name : void 0) !== "String")) {
              for (q = 0, len5 = property.length; q < len5; q++) {
                item = property[q];
